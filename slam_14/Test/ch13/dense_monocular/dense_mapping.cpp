...
const double fy = -481.0f;
const double cx = 319.5f;
const double cy = 239.5f;
const int ncc_window_size = 2; // ncc 取得窗口半径
...
//根据ref中的坐标点，寻找该点在current帧中对应的匹配点
bool epipolarSearch(
        const Mat& ref,
        const Mat& curr,
        const SE3& T_C_R,
...
{
    return Vector2d(
            fx * p_cam(0,0) / p_cam(2,0) + cx,
            fy * p_cam(1,0) / p_cam(2,0) + cy
    );
}
...
// 给定参考帧和对比帧图像，以及参考帧中某待匹配点的像素和对应深度均值、深度的变化方差；求解该像素点在对比帧中对应的极线
bool epipolarSearch(
        ...
        Vector2d& pt_curr)
{
    // depth +- sqrt(depth_cov),3d points in space
    // projection to curr image and get min_pixel and max_pixel coordinates
    // block matching based on ncc, 对比ref的像素块与curr中对应的像素块intensity差别，找到差别最小的像素块
    ...
    double d_min = depth_mu - 3 * depth_cov, d_max = depth_mu + 3 * depth_cov;  //  深度的变化边界
    if (d_min < 0.1) d_min = 0.1;
    Vector2d px_min_curr = cam2px(T_C_R * (f_ref * d_min)); // 按照最小深度投影的像素
    Vector2d px_max_curr = cam2px(T_C_R * (f_ref * d_max)); // 按照最大深度投影的像素
    Vector2d epipolar_line = px_max_curr - px_min_curr;     // 极线（线段形式）
    Vector2d epipolar_direction = epipolar_line;
    epipolar_direction.normalize(); // normalize, divides an object in-place by this norm, the original object itself is modified

    //showEpipolarLine(ref, curr, pt_ref, px_min_curr, px_max_curr);  // 显示极线段，颜色为绿色
...
// 图片中的像素点与BGR或者gray的区别
// Vector2d(x.y) 是像素点的位置，至于这个像素是灰度像素还是彩色像素，要根据channel来确定
bool update(const Mat& ref, const Mat& curr, const SE3& T_C_R, Mat& depth, Mat& depth_cov)
{
    for (int x = boarder; x < width - boarder; ++x)
    {
        for (int y = boarder; y < height - boarder; ++y)
        {
            ...
            if(false == ret)
                continue;
            //showEpipolarMatch(ref, curr, Vector2d(x,y), pt_curr); // 显示匹配的像素点，颜色为红色
...
)
{
    ...
    Vector3d f_ref = px2cam(pt_ref);
    f_ref.normalize();  // divides an object in-place by this norm (i.e. the original object itself is modified).
...
int main(int argc, char** argv)
{
    ...
    for (int index = 1; index < color_image_files.size(); ++index)
    {
        ...
        if (nullptr == curr.data )
            continue;
        SE3 pose_curr_TWC = poses_TWC[index];
        SE3 pose_T_C_R = pose_curr_TWC.inverse() * pose_ref_TWC;    // 以第一帧图像的像素和位姿作为参考帧，不断更新深度。
...
{
    Mat ref_show, curr_show;
    cv::cvtColor(ref, ref_show, CV_GRAY2BGR);   // Gray to BGR?
    cv::cvtColor(curr, curr_show, CV_GRAY2BGR);
    ...
    cv::line(curr_show, Point2f(px_min_curr(0,0), px_min_curr(1,0)),
             Point2f(px_max_curr(0,0), px_max_curr(1,0)), Scalar(0,255,0), 1);  // 0,255,0 是绿色
