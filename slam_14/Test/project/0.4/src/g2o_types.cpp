...
namespace myslam
{
    ...
    {
        g2o::VertexSE3Expmap* pose = static_cast<g2o::VertexSE3Expmap*>(_vertices[1]);  // _vertices[i]是边链接的节点值
        g2o::SE3Quat T(pose->estimate());   // 隐式地调用构造函数，相当于 g2o::SE3Quat T = g2o::SE3Quat(pose->estimate())
        g2o::VertexSBAPointXYZ* point = static_cast<g2o::VertexSBAPointXYZ*> (_vertices[0]);

        // typedef std::vector<Vertex*>  VertexContainer;
        // VertexContainer _vertices;
        // 在边的设置中，通过setVertex(size_t i, Vertex* v)设置节点的序号和值
        ...
        _jacobianOplusXi = -T.rotation().toRotationMatrix();    // P' = R*P + t, P'对P的导数， P'对 Xi的导数： -（I, -P'^）3行，6列
    ...
    {
        const g2o::VertexSE3Expmap* pose = static_cast<const g2o::VertexSE3Expmap*> (_vertices[0]);
        g2o::SE3Quat T(pose->estimate());   // default copy constructor
        Vector3d xyz_trans = T.map(point_);
        double x = xyz_trans[0];
        double y = xyz_trans[1];
        double z = xyz_trans[2];
        ...
        // sum ( [u,v] - (1/s) * K * exp(Xi^)P )^2
        // P' = (exp(Xi^)P)_1:3 = [X',Y',Z']^T, s*Mu = K*P'
